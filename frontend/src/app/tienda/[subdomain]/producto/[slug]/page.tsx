  ...
  return (
    <div className="min-h-screen flex-1 bg-slate-50 pb-24">
      ...
      <main className="mx-auto grid max-w-4xl gap-6 px-4 py-6 md:grid-cols-2">
        ...
        <section className="self-center rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 md:p-8">{product.category && <p className="text-xs font-bold uppercase tracking-[0.14em] text-violet-700">{product.category.name}</p>}<h2 className="mt-2 text-3xl font-black tracking-tight text-slate-950">{product.name}</h2><p className="mt-3 text-3xl font-black text-violet-700">{formatPrice(product.price, store.store.currency)}</p><p className={`mt-2 text-sm font-bold ${product.inStock ? "text-emerald-700" : "text-red-700"}`}>{product.inStock ? `${product.available} disponibles` : "Producto agotado"}</p>{product.description ? <p className="mt-5 whitespace-pre-line text-sm font-medium leading-6 text-slate-700">{product.description}</p> : <p className="mt-5 text-sm font-medium text-slate-500">Sin descripción adicional.</p>}<div className="mt-7"><AddToCart product={product} /></div><p className="mt-4 text-center text-xs font-medium text-slate-500">El precio y el stock se validarán al confirmar el pedido.</p></section>
      </main>
    </div>
  );
}
