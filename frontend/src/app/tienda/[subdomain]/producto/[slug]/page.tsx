...
export async function generateMetadata({
  ...
  try {
    ...
    const description =
      product.description || `Disponible en la tienda. ${product.name}.`;
...
export default async function ProductDetailPage({
  ...
  const reserveHref = phone
    ? `https://wa.me/${phone}?text=${encodeURIComponent(
        `Hola ${store.store.name}, quiero ${action.toLowerCase()}: ${product.name}`,
      )}`
    : null;
      ...
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-4xl items-center gap-3 px-4 py-4">
          <Link
            ...
            className="rounded-lg bg-slate-100 px-3 py-2 font-bold text-slate-700"
          >
            ←
          </Link>
          <div>
            ...
            <h1 className="font-black text-slate-950">Detalle del producto</h1>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto grid max-w-4xl gap-6 px-4 py-6 md:grid-cols-2">
        ...
        <section className="self-center rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 md:p-8">
          ...
          <p className="mt-3 text-3xl font-black" style={{ color: accent }}>
            {formatPrice(product.price, store.store.currency)}
          </p>
          <p
          ...
          <div className="mt-7">
            {usesCart ? (
              ...
              <div className="rounded-xl bg-amber-50 px-4 py-3 text-center text-sm font-semibold text-amber-700">
                Este negocio aún no cargó su WhatsApp.
              </div>
            )}
            <ShareButton title={product.name} accent={accent} />
          </div>
          ...
          <p className="mt-4 text-center text-xs font-medium text-slate-500">
            {usesCart
              ...
              : archetype === "digital"
                ? "El negocio activará tu suscripción y coordinará el pago por WhatsApp."
                : "El negocio confirmará tu cita y coordinará por WhatsApp."}
          </p>
        </section>
      </main>
    </div>
