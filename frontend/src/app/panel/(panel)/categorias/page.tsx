...
export default function CategoriasPage() {
  ...
  async function add() {
    if (name.trim().length < 2) {
      setError("Escribe el nombre de la categoría (mínimo 2 letras) antes de agregar.");
      return;
    }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex max-w-xl gap-2"><input value={name} onChange={(event) => { setName(event.target.value); if (error) setError(""); }} onKeyDown={(event) => event.key === "Enter" && add()} placeholder="Ejemplo: Bebidas" className="h-11 min-w-0 flex-1 rounded-xl border border-slate-300 bg-white px-3 font-medium text-slate-950 outline-none placeholder:text-slate-500 focus:border-violet-600" /><button onClick={add} disabled={saving} className="rounded-xl bg-violet-600 px-5 font-bold text-white hover:bg-violet-700 disabled:opacity-60">Agregar</button></div>
