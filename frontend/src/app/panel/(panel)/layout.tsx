...
export default function PanelLayout({ children }: { children: React.ReactNode }) {
  ...
  // Citas → solo Servicios; Suscripciones → solo Digital. En el resto se ocultan.
  const nav = NAV.filter((item) => {
    if (item.href === "/panel/citas") return usesAppointments;
    if (item.href === "/panel/suscripciones") return usesSubscriptions;
    return true;
  });
  // Barra inferior (móvil): adaptada al rubro.
  const mobileHrefs = usesAppointments
    ? ["/panel", "/panel/pedidos", "/panel/citas", "/panel/productos", "/panel/reportes"]
    : usesSubscriptions
      ? ["/panel", "/panel/pedidos", "/panel/suscripciones", "/panel/productos", "/panel/reportes"]
  ...
  useEffect(() => {
    if (getAccess())
      adminApi
        .settings()
        .then((s) => {
          setSubdomain(s.subdomain ?? null);
          const arch = archetypeOf(resolveCategory(s.businessType));
          setUsesAppointments(arch === "servicios");
          setUsesSubscriptions(arch === "digital");
        })
        .catch(() => {});
  }, []);
  ...
  if (!ready) {
    return <div className="flex min-h-screen items-center justify-center bg-slate-50 font-semibold text-slate-700">Cargando panel...</div>;
  }
      ...
      <aside className="hidden h-dvh w-64 shrink-0 flex-col overflow-y-auto bg-slate-950 px-4 py-5 text-white shadow-xl md:flex">
        <Link href="/panel" className="mb-8 flex items-center gap-3 px-2">
          ...
          <span>
            ...
            <span className="block text-[11px] font-medium uppercase tracking-[0.18em] text-slate-400">Panel del negocio</span>
          </span>
        </Link>
        ...
        <div className="space-y-1.5 border-t border-white/10 pt-4">
          {storeUrl && (
            <a href={storeUrl} target="_blank" rel="noopener noreferrer" className="flex w-full items-center gap-3 rounded-xl bg-white/5 px-3 py-2.5 text-sm font-semibold text-violet-200 transition hover:bg-violet-500/20 hover:text-white">
              <DashboardIcon name="store" />
              Ver mi tienda ↗
            </a>
          )}
          <button onClick={logout} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-300 transition hover:bg-red-500/15 hover:text-red-300">
      ...
      <div className="flex h-dvh min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur md:px-7">
          <div className="flex items-center gap-3 md:hidden">
            <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-violet-600 text-white"><DashboardIcon name="store" /></span>
          ...
          <div className="flex items-center gap-2">
            {storeUrl && (
              <a href={storeUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-lg bg-violet-600 px-3 py-2 text-sm font-bold text-white hover:bg-violet-700">
        ...
        <main className="min-h-0 flex-1 overflow-y-auto p-4 pb-24 text-slate-950 md:p-7 md:pb-7">{children}</main>
        ...
        <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-5 border-t border-slate-200 bg-white px-1 py-2 shadow-[0_-8px_24px_rgba(15,23,42,0.08)] md:hidden">
          {NAV.filter((item) => mobileHrefs.includes(item.href)).map((item) => {
