...
export default function CustomersPage() {
  ...
  return (
    <div className="space-y-5">
      <div><p className="text-sm font-bold text-violet-700">Relaciones</p><h1 className="mt-1 text-xl font-black text-slate-950">Clientes</h1><p className="mt-2 text-sm font-medium text-slate-600">Historial de compradores creado automáticamente desde los pedidos.</p></div>
      <div className="flex max-w-xl gap-2"><input value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => event.key === "Enter" && load()} placeholder="Buscar por nombre o teléfono" className="h-11 min-w-0 flex-1 rounded-xl border border-slate-300 bg-white px-3 text-sm font-medium text-slate-950 outline-none placeholder:text-slate-500 focus:border-violet-600" /><button onClick={() => load()} className="rounded-xl bg-violet-600 px-5 text-sm font-bold text-white hover:bg-violet-700">Buscar</button></div>
